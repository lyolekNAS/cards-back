...
public interface StateLimitRepository extends JpaRepository<StateLimit, Integer> {
	...
	@Query("SELECT new org.sav.cardsback.dto.StateLimitDto(s.stateId, s.attempt, s.delay, s.color) FROM StateLimit s")
	public List<StateLimitDto> findAllStateLimitDtos();

	@Query("SELECT new org.sav.cardsback.dto.StateLimitDto(s.stateId, s.attempt, s.delay, s.color) FROM StateLimit s WHERE s.stateId = :id")
