...
public class AiSynonymMiner {
	...
	private void run() {
		try {
			log.debug(">>>> Starting AiSynonymMiner");
			wordProcessingService.findWordWithoutAiSynonyms()
					.ifPresentOrElse(
							w -> {
								log.debug(">>>> AiSynonymMiner: {}", w.getWordText());
								wordProcessingService.enrichWithAiSynonyms(w);
							},
							() -> log.debug(">>>> AiSynonymMiner on the rest")
					);
		} finally {
			scheduleNext();
		}
